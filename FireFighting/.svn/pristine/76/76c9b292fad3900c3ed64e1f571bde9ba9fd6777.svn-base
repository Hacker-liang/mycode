...
- (NSData *)EncodeWithInput
{
    ...
    NSString *department;
    if (_departmentSegment.selectedSegmentIndex == 1)
        department = @"all";
    else department = _userInfo.department;
    [requesDic setObject:_userInfo.userNo forKey:@"userno"];
    [requesDic setObject:_userInfo.userName forKey:@"username"];
    [requesDic setObject:department forKey:@"todepartment"];
    [requesDic setObject:_userInfo.department forKey:@"fromdepartment"];
